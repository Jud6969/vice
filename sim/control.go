...
func (s *Sim) RunAircraftControlCommands(tcw TCW, callsign av.ADSBCallsign, commandStr string, audioDuration time.Duration) ControlCommandsResult {
	...
	for i, command := range commands {
		intent, err := s.runOneControlCommand(tcw, callsign, command, delayReduction, false)
		if err != nil {
...
// NNN.NN (an implicit trailing zero is appended, giving NNNNN0 kHz); 6 digits
// are NNN.NNN kHz directly. The resulting frequency must be in the aviation
// band [118000, 137000] kHz inclusive. Returns ErrInvalidCommandSyntax on
// length/format failure and ErrInvalidFrequency on out-of-band values.
