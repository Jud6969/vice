...
func (s *Sim) RunAircraftControlCommands(tcw TCW, callsign av.ADSBCallsign, commandStr string, audioDuration time.Duration) ControlCommandsResult {
	...
	// An altimeter issued alongside an ATIS letter is the setting the pilot
	// ends up on, regardless of which came first in the transmission.
	explicitAltimeter := slices.ContainsFunc(commands, isAltimeterCommand)

	for i, command := range commands {
		intent, err := s.runOneControlCommand(tcw, callsign, command, delayReduction, explicitAltimeter)
		if err != nil {
...
// explicitAltimeter says whether the transmission this command came from also
// carried an altimeter setting, which suppresses the one an ATIS would apply.
func (s *Sim) runOneControlCommand(tcw TCW, callsign av.ADSBCallsign, command string, delayReduction time.Duration,
	explicitAltimeter bool) (av.CommandIntent, error) {
	if len(command) == 0 {
		return nil, ErrInvalidCommandSyntax
	}
	...
	case 'A':
		if command == "A" {
			...
			return s.SetAltimeter(tcw, callsign, float32(v)/100)
		} else if letter, ok := strings.CutPrefix(command, "ATIS/"); ok {
			return s.ATISCommand(tcw, callsign, letter, explicitAltimeter)
		} else {
