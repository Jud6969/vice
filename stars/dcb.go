...
func (sp *STARSPane) dcbButtonScale(ctx *panes.Context) float32 {
	ps := sp.currentPrefs()
	numDCBSlots := float32(sp.dcbSlots())
...
func (sp *STARSPane) drawDCBButton(ctx *panes.Context, text string, flags dcbFlags, buttonScale float32, pushedIn bool) bool {
	...
	// Text last!
	if disabled {
		dcbDrawState.cb.EnablePolygonStipple()
		dcbDrawState.cb.PolygonStipple(disabledButtonTextStipple)
	}
	td.GenerateCommands(dcbDrawState.cb)
	if disabled {
		dcbDrawState.cb.DisablePolygonStipple()
	}
...
var disabledButtonTextStipple [32]uint32 = [32]uint32{
