...
func (a ApproachIntent) Render(rt *RadioTransmission, r *rand.Rand) {
	...
	case ApproachAtFixCleared:
		if a.StraightIn {
			rt.Add("at {fix} cleared straight in {appr}", a.Fix, a.ApproachName)
		} else {
...
// does NOT switch frequencies; the initial readback acknowledges the
// frequency, and the caller is expected to enqueue a deferred "nothing
// heard" callback a few seconds later.
type UnknownFrequencyIntent struct {
	Frequency Frequency
}

func (u UnknownFrequencyIntent) Render(rt *RadioTransmission, r *rand.Rand) {
	rt.Add("[{freq}|switching {freq}|over to {freq}|going over to {freq}]", u.Frequency)
}
